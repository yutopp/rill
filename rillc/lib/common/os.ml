...
let exec args ~f =
  ...
  f out_ch err_ch status

let assume_exit_successfully ~status =
  match status with
  | Unix.WEXITED 0 -> Ok ()
  | s -> Error (Not_exited_with_code_zero s)

let mktemp_dir prefix =
  let open Result.Let_syntax in
  let tmp = Caml.Filename.get_temp_dir_name () in
  exec [ "mktemp"; "-d"; "-p"; tmp; "-t"; prefix ]
    ~f:(fun out_ch err_ch status ->
      let%bind () = assume_exit_successfully ~status in
      let out =
...
let cc_obj src out =
  ...
  exec args ~f:(fun _out_ch _err_ch status ->
      let%bind () = assume_exit_successfully ~status in
      Ok ())
